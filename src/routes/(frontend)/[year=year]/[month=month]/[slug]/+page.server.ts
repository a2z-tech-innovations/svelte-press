...
import { loadPostBySlug } from '$lib/server/postLoader.js';
...
export const load: PageServerLoad = ({ params }) => {
	...
	const result = loadPostBySlug(slug);

	if (!result) {
		error(404, 'Post not found');
	}

	// Validate that the post date actually matches the year/month in the URL
	const postDate = result.post.postDate ? new Date(result.post.postDate) : null;
	if (!postDate) {
		error(404, 'Post not found');
	}
