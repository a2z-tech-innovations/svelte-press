<script lang="ts">
	...
	function renderBlock(block: { id?: string; type: string; content: string; attrs: Record<string, unknown> }): string {
		switch (block.type) {
			...
			case 'image': return '<figure><img src="' + block.attrs.src + '" alt="' + (block.attrs.alt || '') + '" /></figure>';
			case 'gallery': {
				...
				const thumbs = (imgs as Array<{ src: string; alt?: string; caption?: string }>)
					.map((img, idx) => {
						const caption = img.caption ? '<span class="sp-gallery-grid-caption">' + img.caption + '</span>' : '';
						return '<button type="button" class="sp-gallery-grid-btn" data-sp-gallery-id="' + blockId + '" data-sp-gallery-index="' + idx + '" aria-label="View image ' + (idx + 1) + ' of ' + imgs.length + '"><img src="' + img.src + '" alt="' + (img.alt ?? '') + '" class="sp-gallery-grid-img" loading="lazy" />' + caption + '</button>';
					}).join('');
				return '<div class="sp-gallery-grid" data-sp-gallery="' + blockId + '">' + thumbs + '</div>';
			}
			case 'quote': return '<blockquote>' + block.content + '</blockquote>';
			case 'list': return '<' + (block.attrs.ordered ? 'ol' : 'ul') + '>' + block.content + '</' + (block.attrs.ordered ? 'ol' : 'ul') + '>';
			...
			case 'embed': {
				...
				if (embedHtml) return '<div class="wp-embed-block">' + embedHtml + '</div>';
				if (embedUrl) return '<div class="wp-embed-block"><a href="' + embedUrl + '" target="_blank" rel="noopener noreferrer">' + embedUrl + '</a></div>';
				return '';
			}
	...
	let galleryBlocks = $derived(
		Array.isArray(data.post.content)
			? (data.post.content as Array<{ id: string; type: string; attrs: Record<string, unknown> }>).filter((b) => b.type === 'gallery')
			: []
	);
...
<article class="fp-single">
	<header class="fp-single-header">
		...
		<div class="fp-single-meta">
			...
			{#if data.post.authorName}
				...
				<a href="/author/{data.post.authorUsername}" class="fp-single-author">{data.post.authorName}</a>
			{/if}
	...
	{#if data.post.authorName}
		<div class="fp-author-box">
			<img src={data.post.authorAvatarUrl} alt={data.post.authorName} class="fp-author-avatar" width="72" height="72" />
			<div class="fp-author-info">
				<a href="/author/{data.post.authorUsername}" class="fp-author-name">{data.post.authorName}</a>
...
<section class="fp-comments" id="comments">
	<h2 class="fp-comments-title">
		{#if data.comments.length === 0}No comments yet{:else}{data.comments.length} {data.comments.length === 1 ? 'Comment' : 'Comments'}{/if}
	</h2>
	...
	{#if data.post.commentStatus === 'open'}
		<div class="fp-comment-form-wrap" id="comment-form">
			<h3 class="fp-comment-form-title">{replyToId ? `Reply to ${replyToName}` : 'Leave a Comment'}</h3>
			{#if replyToId}
			...
			{/if}
			{#if form?.success}
				<div class="fp-notice fp-notice--success">{form.message ?? 'Your comment has been submitted and is awaiting moderation.'}</div>
			{:else if form?.error}
				<div class="fp-notice fp-notice--error">{form.error}</div>
			{/if}
			<form method="post" action="?/comment" class="fp-comment-form" use:enhance>
				...
				<div class="fp-form-row">
					<div class="fp-form-field">
						...
						<input id="comment-name" type="text" name="name" bind:value={commentName} required class="fp-form-input" autocomplete="name" />
					</div>
					<div class="fp-form-field">
						<label for="comment-email" class="fp-form-label">Email <span aria-hidden="true">*</span></label>
						<input id="comment-email" type="email" name="email" bind:value={commentEmail} required class="fp-form-input" autocomplete="email" />
						<p class="fp-form-hint">Your email will not be published.</p>
					</div>
				</div>
				<div class="fp-form-field">
					<label for="comment-url" class="fp-form-label">Website</label>
					<input id="comment-url" type="url" name="url" bind:value={commentUrl} class="fp-form-input" autocomplete="url" />
				</div>
				<div class="fp-form-field">
					<label for="comment-content" class="fp-form-label">Comment <span aria-hidden="true">*</span></label>
					<textarea id="comment-content" name="content" bind:value={commentContent} required rows="6" class="fp-form-textarea"></textarea>
				</div>
...
<style>
	...
	.fp-single-content :global(h2) { font-size: 1.6rem; }
	.fp-single-content :global(h3) { font-size: 1.3rem; }
	.fp-single-content :global(h4) { font-size: 1.1rem; }
	.fp-single-content :global(blockquote) { border-left: 3px solid #1d2327; padding-left: 1.25rem; margin: 1.5rem 0; color: #3c434a; font-style: italic; }
	.fp-single-content :global(pre) { background: #f6f7f7; border: 1px solid #e8e8e8; border-radius: 4px; padding: 1rem 1.25rem; overflow-x: auto; margin-bottom: 1.5rem; }
	.fp-single-content :global(code) { font-family: Consolas, monospace; font-size: 0.875em; }
	...
	.fp-tag { padding: 0.2rem 0.6rem; border: 1px solid #c3c4c7; border-radius: 2rem; font-size: 0.75rem; color: #646970; text-decoration: none; }
	...
	.fp-author-name { display: block; font-weight: 700; color: #1d2327; text-decoration: none; margin-bottom: 0.25rem; }
	...
	.fp-comments-title { font-size: 1.5rem; font-weight: 700; color: #1d2327; margin-bottom: 1.5rem; }
	...
	.fp-reply-indicator { background: #f0f0f1; border-left: 3px solid #2271b1; padding: 8px 12px; font-size: 0.8125rem; margin-bottom: 12px; border-radius: 0 4px 4px 0; }
	...
	.fp-comment-form-title { font-size: 1.25rem; font-weight: 700; margin-bottom: 1.25rem; }
