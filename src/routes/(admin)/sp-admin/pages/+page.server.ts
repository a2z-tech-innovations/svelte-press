...
export const load: PageServerLoad = async ({ url }) => {
	...
	const baseConditions = [eq(posts.postType, 'page')];
	...
	const allCounts = db
		...
		.from(posts)
		.where(and(eq(posts.postType, 'page'), search ? like(posts.title, `%${search}%`) : sql`1=1`))
	...
	const pageList = db
		...
		.where(whereClause)
		.orderBy(posts.menuOrder, desc(posts.modifiedDate))
		.limit(PER_PAGE)
		.offset(offset)
		.all();
...
export const actions: Actions = {
	bulk: async ({ request }) => {
		...
		const ids = data.getAll('pageIds').map((v) => Number(v)).filter(Boolean);
